...
export interface RichTextEditorStrategy {
   ...
   * string.
   *
   * - HTML strategy: clones the DOM, replaces template block UI
   *   nodes with XML blocks, returns `innerHTML`.
   * - Markdown strategy: returns the editor's Markdown text with
   *   XML template blocks intact.
   */
  serialiseContent(
   ...
   *
   * - HTML strategy: expands XML template blocks to chip/container markup.
   * - Markdown strategy: converts Markdown → HTML, then expands
   *   XML placeholder blocks.
   */

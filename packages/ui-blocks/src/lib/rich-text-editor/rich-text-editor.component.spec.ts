...
describe("UIRichTextEditor", () => {
  ...
  describe("value binding", () => {
    ...
    it("should serialise edited HTML as XML-compatible markup", () => {
      const editor: HTMLDivElement =
        fixture.nativeElement.querySelector(".editor");
      editor.innerHTML = "<p>A<br>B</p><hr><p>C</p>";
      editor.dispatchEvent(new Event("input"));
      fixture.detectChanges();

      expect(component.value()).toContain("<br />");
      expect(component.value()).toContain("<hr />");
      expect(component.value()).not.toContain("<br>");
      expect(component.value()).not.toContain("<hr>");
    });

    it("should preserve table row loops as processable XML", () => {
      const template =
        '<table><tbody><loop items="lines"><tr><td><placeholder key="description" /></td></tr></loop></tbody></table>';
      (component as any).renderToEditor(template);
      fixture.detectChanges();
      ...
      expect(component.value()).toContain(
        '<tbody><loop items="lines"><tr><td><placeholder key="description" /></td></tr></loop></tbody>',
      );
      const rendered = new TextTemplateProcessor().expand(component.value(), {
        lines: [{ description: "Alpha" }, { description: "Beta" }],
      });
      expect(rendered).toContain("<td>Alpha</td>");
      expect(rendered).toContain("<td>Beta</td>");
    });
  });
  ...
  describe("placeholder picker", () => {
    ...
    it("should render placeholder picker from context data shape", () => {
      fixture.componentRef.setInput("placeholders", []);
      ...
      const paths = Array.from<Element>(
        fixture.nativeElement.querySelectorAll(".placeholder-path"),
      ).map((el) => (el.textContent ?? "").trim());
      expect(labels).toContain("fullName");
      expect(labels).toContain("lines[]");
      expect(paths).not.toContain("lines[].description");
      expect(paths).not.toContain("lines[].quantity");
    });

    it("should render loop item placeholders when the caret is inside a loop", () => {
      fixture.componentRef.setInput("placeholders", []);
      fixture.componentRef.setInput("placeholderContext", {
        fullName: "Alice Hartwell",
        lines: [
    ...
    it("should insert loop item placeholders using the item-relative key", () => {
      fixture.componentRef.setInput("placeholders", []);
      fixture.componentRef.setInput("placeholderContext", {
        lines: [
          {
      ...
      expect(component.value()).toContain('<placeholder key="description" />');
  ...
  describe("placeholder serialisation", () => {
    it("should serialise placeholder chips as XML blocks", () => {
      const editor: HTMLDivElement =
        fixture.nativeElement.querySelector(".editor");
      editor.innerHTML =
        'Hello <span class="rte-placeholder rte-template-block" contenteditable="false" data-template-block="placeholder" data-template-self-closing="true" data-template-attr-key="firstName" data-placeholder-key="firstName">First Name</span>!';
      editor.dispatchEvent(new Event("input"));
      fixture.detectChanges();
    ...
    it("should deserialise placeholder XML blocks into chips", () => {
      fixture.componentRef.setInput("placeholders", [
    ...
    it("should deserialise keys containing hyphens, dots, and colons", () => {
      ...
      (component as any).renderToEditor(
        '<placeholder key="first-name" /> <placeholder key="contact.email" /> <placeholder key="ns:id" />',
      );
      fixture.detectChanges();
    ...
    it("should re-render editor when value changes externally", () => {
      ...
      component.value.set('<p>Hello <placeholder key="firstName" /></p>');
      fixture.detectChanges();
    ...
    it("should preserve placeholder attrs through sanitiseHtml", () => {
      const chipHtml =
        '<span class="rte-placeholder rte-template-block" contenteditable="false" data-template-block="placeholder" data-template-self-closing="true" data-template-attr-key="firstName" data-placeholder-key="firstName">First Name</span>';
      const sanitised = (component as any)
        .strategy()
        .sanitiseHtml(`<p>Hello ${chipHtml}</p>`);

      expect(sanitised).toContain('data-placeholder-key="firstName"');
      expect(sanitised).toContain('data-template-block="placeholder"');
      expect(sanitised).toContain('data-template-attr-key="firstName"');
      expect(sanitised).toContain('contenteditable="false"');
      expect(sanitised).toContain('class="rte-placeholder rte-template-block"');
    });

    it("should render container template blocks with generic block UI", () => {
      (component as any).renderToEditor(
        '<if test="show">Hello <placeholder key="firstName" /></if>',
      );
      fixture.detectChanges();
  ...
  describe("source editing", () => {
    ...
    it("should update WYSIWYG preview when source edits are committed", () => {
      const btn: HTMLButtonElement = fixture.nativeElement.querySelector(
        '[aria-label="Edit HTML source"]',
      );
      btn.click();
  ...
  describe("template block attribute editor", () => {
    let mockPopoverRef: PopoverRef<any>;
    let openPopoverSpy: ReturnType<typeof vi.fn>;

    beforeEach(() => {
      mockPopoverRef = new PopoverRef<any>();
    ...
    it("should offer one-level array context options for loop items", () => {
      fixture.componentRef.setInput("placeholderContext", {
        fullName: "Alice Hartwell",
        lines: [],
        customer: {
      ...
      (component as any).openTemplateBlockDialog(
        "loop",
        document.createElement("button"),
      );

      const config = openPopoverSpy.mock.calls[0][0];
      const fields = config.inputs.attributeDefinitions;
      const itemsField = fields.find((field: any) => field.key === "items");
      expect(itemsField.type).toBe("select");
      expect(itemsField.options).toEqual([{ value: "lines", label: "lines" }]);
    });

    it("should keep template block popovers open until explicitly closed", () => {
      (component as any).openTemplateBlockDialog(
        "loop",
        document.createElement("button"),
      );
    ...
    it("should offer context placeholder options for placeholder keys", () => {
      ...
      const config = openPopoverSpy.mock.calls[0][0];
      const fields = config.inputs.attributeDefinitions;
      const keyField = fields.find((field: any) => field.key === "key");
      expect(keyField.type).toBe("select");
      expect(keyField.options).toContainEqual({
        value: "fullName",
        label: "fullName",
      });
      expect(keyField.options).not.toContainEqual({
        value: "description",
        label: "description",
      });
    });

    it("should offer loop item placeholder key options when scoped inside a loop", () => {
      fixture.componentRef.setInput("placeholderContext", {
        lines: [
          {
            description: "Professional License",
    ...
    it("should use the active loop item context for nested loop choices", () => {
      fixture.componentRef.setInput("placeholderContext", {
        lines: [
          {
            description: "Professional License",
      ...
      const fields = config.inputs.attributeDefinitions;
      const itemsField = fields.find((field: any) => field.key === "items");
      expect(itemsField.options).toEqual([

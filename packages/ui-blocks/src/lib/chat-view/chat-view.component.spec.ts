...
describe('UIChatView', () => {
  ...
  describe('message rendering', () => {
    ...
    it('should mark own messages with --mine class', () => {
      ...
      expect(messageEls[0].classList.contains('chat-message--mine')).toBe(false);
    ...
    it('should render rich-text messages with innerHTML', () => {
      const msgs = [
        makeMessage('1', bob, '<strong>Bold</strong> text', { type: 'rich-text' }),
      ];
      fixture.componentRef.setInput('messages', msgs);
      fixture.detectChanges();
  ...
  describe('composer', () => {
    ...
    it('should disable send button when composer is empty', () => {
      const el: HTMLElement = fixture.nativeElement;
      const sendBtn = el.querySelector('.chat-send-button') as HTMLButtonElement;

...
export class RestDatasource<T> implements IDatasource<T> {
    ...
    private async fetchPage(pageIndex: number): Promise<T[]> {
        ...
        if (!response.ok) {
            throw new Error(`Failed to load page ${pageIndex} (HTTP ${response.status})`);
        }
    ...
    private trySetTotalItems(payload: RestPayload<T>, pageIndex: number, itemCount: number): void {

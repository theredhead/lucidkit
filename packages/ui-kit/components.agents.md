...
| `UIIndicatesTouch`      | `src/lib/indicates-touch/indicates-touch.directive.ts`                        | `[uiIndicatesTouch]`        | Directive — adds animated ripple feedback on pointer interaction; colour via `--ui-ripple-color`                                   |
...
| `UISplitContainer`      | `src/lib/split-container/split-container.component.ts`                        | `ui-split-container`        | Resizable N-panel split container with draggable dividers between each adjacent pair                                               |
| `UISplitPanel`          | `src/lib/split-container/split-panel.component.ts`                            | `ui-split-panel`            | Child panel for `<ui-split-container>` — accepts optional `[min]`/`[max]` constraints                                              |
...
| Name                | File                                                                     | Selector               | Description                                                                                                                                    |
| ------------------- | ------------------------------------------------------------------------ | ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| `UIToolbar`         | `src/lib/toolbar/toolbar.component.ts`                                   | `ui-toolbar`           | Container that collects `UIToolbarItem` children, re-emits their actions via `toolAction`, and supports inline or floating-toggle presentation |
...
| `UISpacerTool`      | `src/lib/toolbar/tools/spacer-tool/spacer-tool.component.ts`             | `ui-spacer-tool`       | Flexible spacer (`flex: 1 1 auto`) that pushes subsequent items to the trailing edge                                                           |
...
| `ToolbarDisplayMode` | `src/lib/toolbar/toolbar.component.ts` | `'inline' \| 'floating-toggle'`                                     |
